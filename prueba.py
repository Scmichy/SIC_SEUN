...
class MainApp(App):
    def build(self):
        ...
        # Widget para mostrar la imagen de la cámara
        self.image_widget = Image()
        self.layout.add_widget(self.image_widget)

        # Panel de texto no modificable
        self.text_panel = TextInput(
            readonly=True,  # Hace que el texto no sea modificable
            multiline=True,  # Permite múltiples líneas de texto
            size_hint=(1, 0.3),  # Tamaño  panel de texto
            background_color=(0.9, 0.9, 0.9, 1),  # Color de fondo
            foreground_color=(0, 0, 0, 1),  # Color del texto
            font_size=20  # Tamaño de la fuente
        )
        self.layout.add_widget(self.text_panel)

        # Botón para agregar la letra detectada
    ...
    def update(self, dt):
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                # Dibujar los landmarks (las lineas y conexiones de manos)
    ...
    # Función para deteccion de letras
